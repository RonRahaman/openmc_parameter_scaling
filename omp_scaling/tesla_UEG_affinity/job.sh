...
while [ $i_rep -lt $n_reps ]; do
  ...
  # Command to run
  CMD="GOMP_AFFINITY='${AFFINITIES[$i_thread]}' $EXEC_PATH --threads ${THREADS[$i_thread]} --resize-egrid ${SIZES[$i_size]} --energy-grid union $SETTINGS > $PWD/$OUT_PREFIX.output 2>&1"
